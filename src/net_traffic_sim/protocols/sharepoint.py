...
# ---
# HTTP SharePoint traffic generator (port 8080).
# ---
...
# def generate_http_file_download(src_ip, dst_ip, sport, filename, filesize, start_time):
# """Generate HTTP file download from SharePoint with realistic file headers"""
# packets = []
# current_time = start_time
...
# pkt = create_tcp_psh_ack(src_ip, dst_ip, sport, 8080,
# conn.seq, conn.ack, 65535, request, current_time)
# packets.append(pkt)
# conn.seq += len(request)
# current_time += exponential_delay(0.010, 0.050, 50)
...
# pkt = create_tcp_psh_ack(dst_ip, src_ip, 8080, sport,
# conn_server.seq, conn.seq, 65535,
# response_headers_bytes, current_time)
# packets.append(pkt)
# conn_server.seq += len(response_headers_bytes)
# current_time += exponential_delay(0.005, 0.015, 100)

# # Client ACK
# pkt = create_tcp_ack(src_ip, dst_ip, sport, 8080,
# conn.seq, conn_server.seq, 65535, current_time)
# packets.append(pkt)
# conn.ack = conn_server.seq
# current_time += exponential_delay(0.001, 0.005, 200)
...
# # Note: Server sends file with TCP SLOW START pattern
# bytes_sent = 0
# chunk_size = 536 # Start with small chunk (TCP slow start)
# first_chunk = True
# packet_count = 0

# while bytes_sent < filesize:
# # REALISTIC: Grow chunk size (TCP congestion window growth)
# if bytes_sent < 65536: # First 64KB - slow start phase
# chunk_size = min(chunk_size + random.randint(50, 200), 1460)
# else: # After slow start - steady state
# chunk_size = random.choice([1380, 1448, 1460])

# # Add 15% chance of smaller fragments (network jitter/fragmentation)
# if random.random() < 0.15:
# chunk_size = random.randint(200, 800)

# # Calculate actual chunk size (don't exceed file size)
# chunk = min(chunk_size, filesize - bytes_sent)
...
# first_chunk = False
# else:
# # Note: Generate realistic binary content (not just zeros)
# chunk_data = bytes([random.randint(0, 255) for _ in range(chunk)])

# pkt = create_tcp_psh_ack(dst_ip, src_ip, 8080, sport, conn_server.seq, conn.seq, 65535, chunk_data, current_time)

# # Fragment large HTTP chunks (very common for file transfers)
# if len(chunk_data) > 1460:
# frag_packets = fragment_large_payload(pkt, mss=1460)
# packets.extend(frag_packets)
# conn_server.seq += chunk
...
# ---
# RDP TRAFFIC GENERATOR (TLSv1.2 on port 3389)
# ---
...
# ---
# SMB file transfer traffic generator.
# ---
...
# ---
# KERBEROS AND LDAP TRAFFIC GENERATOR
# ---
...
def generate_kerberos_ldap_traffic(start_time, duration, serializer: FastPacketSerializer | None = None):
    ...
    while current_time < start_time + duration and auth_count < target_auths:
        ...
        if random.random() < 0.5:  # 50% Kerberos
            ...
            realistic_username = random.choice(REALISTIC_USERS)  # <- jsmith, agarcia, etc
...
# ---
# ADDITIONAL PROTOCOL GENERATORS
# ---
